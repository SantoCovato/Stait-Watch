...
Stait Watch is an open-source firmware for the **ESP32-S3** platform, specifically designed to drive **1.28-inch circular touch displays** (Waveshare style). It enables real-time interaction with an Android companion app via Bluetooth Low Energy (BLE), managing notifications, weather, and system settings.
...
The interface is completely gesture-based, allowing you to swipe between the watchface, media controls, weather forecasts, and system settings.
...
* **Compatibility**: This code has been tested and optimized **exclusively for the Waveshare ESP32-S3 1.28" Touch Display**. Using it on different hardware layouts will require modifications to the `config.h` and pin definitions in the main code.

## 🚀 Companion App & Ecosystem
This firmware works in tandem with **Stait Hub**, the official Android companion application developed in Flutter. Stait Hub handles system-level notification filtering, background connectivity, and weather data pushing.

* **App Repository**: [Insert Link to Stait Hub Repository Here]

## 💻 Configuration and Compilation
This project is developed to be compiled using **PlatformIO** (recommended) or **Arduino IDE**.
...
### Setup
1. Clone this repository: `git clone https://github.com/your-username/stait-watch.git`
